...
def merge_func(i, dest, debug=0):
  pid = os.getpid()
  fname = "%d-%s" % (pid, dest)
  if i.size > 0: 
    with open(fname, 'a') as fh:
      i.get_contents_to_file(fh)
      fh.write("\n")
  return
...
def main():
  ...
  full_list = get_files(args.src, args.debug)
  ...
  if args.cmd == 'copy':
    ...
    pool.map(envoke_merge, itertools.izip(list_to_process, itertools.repeat(args.dst), itertools.repeat(args.debug)))
  else:
